  ...
  try {
    ...
    if (await driverHandler.checkDriver(Driver.username)) {
      ...
      if (result.newPassanger) {
        ...
        response(res, 200, {
          Message: resmessage.update_status_found_passanger,
          passangers: result.data,
          newPassanger: true,
        });
      } else
        response(res, 200, {
          Message: resmessage.update_status_no_passanger,
          newPassanger: false,
        });
    } else {
      response(res, 202, { Message: resmessage.user_not_found });
...
routes.post("/findDriver/:username", async (req, res, next) => {
  try {
    ...
    if (driver.driver) {
      ...
      let newJourney = await passangersHandler.addJourney(
        Passanger,
        driver.data
      );
      response(res, 200, {
        Message: resmessage.driver_found,
        driverData: driver.data,
        journeyData: newJourney,
      });
