...
export const getDriver = async(params) => {
    ...
    try{
        ...
        for(let username in activeDrivers){
            let distance = await findDistance(activeDrivers[username]["location"],params.location)
            let tripDistance = await findDistance(params.ending_point,params.starting_point)
